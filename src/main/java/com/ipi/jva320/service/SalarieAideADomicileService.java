...
public class SalarieAideADomicileService {
    ...
    public Page<SalarieAideADomicile> getSalaries(String nom, Pageable pageable) {
        ...
        if (getSalaries(nom).size() < startItem) {
            list = Collections.emptyList();
        } else {
            int toIndex = Math.min(startItem + pageSize, getSalaries().size());
            list = getSalaries().subList(startItem, toIndex);
        }

        Page<SalarieAideADomicile> salariePage = new PageImpl<SalarieAideADomicile>(list,
                PageRequest.of(currentPage, pageSize), getSalaries().size());
    ...
    public void clotureMois(SalarieAideADomicile salarieAideADomicile, double joursTravailles) throws SalarieException {
        // incrémente les jours travaillés de l'année N du salarié de celles passées en
        // paramètres
